...
def save_to_csv_login(tweets, path):
    ...
    with open(path, mode='a', newline='', encoding='utf-8') as file:
        ...
        for tweet in tweets:
            reply_count, repost_count, like_count, bookmark_count, view_count, reply_to_id, url, time_stamp = tweet.get_stats()
            spreading_rate = 0 if view_count == 0 else round((reply_count + repost_count) / view_count * 100, 3)
            writer.writerow([url, reply_to_id, time_stamp, reply_count, repost_count, like_count, bookmark_count, view_count, spreading_rate])
...
class Tweet:
    def __init__(self, reply_count: int, repost_count: int, like_count: int, bookmark_count: int, view_count: int, reply_to_url: str, url: str, time_stamp: str):
    ...
    def get_stats(self):
        return self.reply_count, self.repost_count, self.like_count, self.bookmark_count, self.view_count, self.reply_to_id, self.url, self.time_stamp
